...
class Leveler(commands.Cog):
    ...
    async def setlevel(self, ctx, user: discord.Member, level: int):
        ...
        userinfo["servers"][str(server.id)]["current_exp"] = old_server_exp
    ...
    async def draw_profile(self, user, server):
        ...
        draw.text(
            (await self._center(0, 140, "    RANK", label_fnt), label_align - 30),
            "    RANK",
            font=label_fnt,
            fill=info_text_color,
        )  # Rank
        draw.text(
            (await self._center(0, 340, "    LEVEL", label_fnt), label_align - 30),
            "    LEVEL",
            font=label_fnt,
            fill=info_text_color,
        )  # Exp
        draw.text(
            (await self._center(200, 340, "Computers", label_fnt), label_align - 30),
            "Computers",
            font=label_fnt,
            fill=info_text_color,
        )  # Credits
    ...
    async def draw_rank(self, user, server):
        ...
        draw.text(
            (await self._center(100, 200, server_rank, large_fnt), v_label_align - 25),
            server_rank,
            font=large_fnt,
            fill=info_text_color,
        )  # Rank
        ...
        draw.text(
            (await self._center(95, 360, level_text, large_fnt), v_label_align - 25),
            level_text,
            font=large_fnt,
            fill=info_text_color,
        )  # Level
        ...
        draw.text(
            (await self._center(260, 360, credit_txt, large_fnt), v_label_align - 25),
            credit_txt,
            font=large_fnt,
            fill=info_text_color,
        )  # Balance
...
# total_exp = await self._level_exp(level)
    async def _level_exp(self, total_exp):
        ...
        level = 0
        while remaining_xp > await self._required_exp(level):
#        while remaining_xp > self._level_exp(level):
